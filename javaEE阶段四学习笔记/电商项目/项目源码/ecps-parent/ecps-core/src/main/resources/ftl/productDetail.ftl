...
function addCart(){
    ...
    if(result){
        window.location.href="http://localhost:8089/cart/addCart.do?skuId="+skuId+"&quantity="+quantity;
    }else {
        alert("亲，实在很抱歉，单前规格的商品库存不足了！");
    }
					...
					<li><label>规　　格：</label><div class="pre spec">
					   <#list item.skuList as sku>
                        ...
                        <li><label>我 要 买：</label>
                            ...
                            <input readonly type="text" id="quantity" name="quanity" value="1" class="num" size="3" />
                            <a href="javascript:void(0);" class="inb add"></a>
                            <em id="sub_add_msg" class="red"></em></li>
                        <li class="submit">
						<input id="buy" type="button" value="" class="hand btn138x40" onclick="buy();"/>
						<input id="myCart" type="button" value="" class="hand btn138x40b" onclick="addCart()"/>
