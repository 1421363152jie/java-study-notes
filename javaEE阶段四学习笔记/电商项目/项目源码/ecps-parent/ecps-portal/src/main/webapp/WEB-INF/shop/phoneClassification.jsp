...
<script type="text/javascript">
	function addBrowse(itemId,itemName,skuPrice,imgs) {
        $.ajax({
            url:"${path}/cart/addBrowse.do",
            type:"post",
            data:{
                itemId:itemId,
                itemName:itemName,
	...
	<div class="bg_gray mt ofc">
			...
			<ul class="uls i_150x150 x4_150x150b">
				...
				<c:forEach items="${itemList}" var="item">
					<li>
						<input type="hidden" id="imgs" value="${item.imgs}">
						<input type="hidden" id="skuPrice" value="${item.skuPrice}">
						<a href="${path}/html/${item.itemId}.html"  target="_blank" class="pic"><img onclick="addBrowse(${item.itemId},'${item.itemName}','${item.skuPrice}','${item.imgs}')" src="${request_file_path}${item.imgs}" /></a>
						<dl>
							<!-- dt 10个文字+... -->
							<dt><a href="./productDetail.jsp" id="itemName"   target="_blank">${item.itemName}</a></dt>
							<!-- dt 25个文字+... -->
							<dd class="h40" style="height: 20px">${item.promotion}</dd>
