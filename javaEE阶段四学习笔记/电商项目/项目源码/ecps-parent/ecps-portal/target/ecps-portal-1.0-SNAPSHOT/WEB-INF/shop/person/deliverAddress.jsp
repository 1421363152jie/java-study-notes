			...
			<div class="ofc">
				...
				<c:forEach items="${addrList}" var="addr">
					<tr <c:if test="${addr.defaultAddr==1}">class='here' </c:if>>
						...
						<td>${addr.zipCode}</td>
						<td>${addr.phone}</td>
						<td class="def"><a href="javascript:void(0);" onclick="setDefaultAddr(${addr.shipAddrId})" title="设为默认">设为默认</a></td>
						<td><a href="javascript:void(0);" title="修改" onclick="modify(${addr.shipAddrId})" class="blue">[修改]</a><a href="javascript:void(0);" title="删除" onclick="del(${addr.shipAddrId})" class="blue">[删除]</a></td>
					</tr>
				</c:forEach>
				</tbody>
				</table>
				...
				<form id="jvForm" action="${path}/user/login/savaOrUpdateByAddr.do" method="post">
