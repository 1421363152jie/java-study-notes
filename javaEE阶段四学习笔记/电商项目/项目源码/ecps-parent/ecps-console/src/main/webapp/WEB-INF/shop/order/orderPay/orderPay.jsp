...
$(function(){	
	var isCall=parseInt($("#isCall").val());
	/*switch(orderState){
...
<div class="frameR"><div class="content">
    ...
    <h2 class="h2_ch">
      <span id="tabs" class="l">
        <a id="label1" href="${path}/order/listPayOrder.do?assignee=noPaidOrderer&isCall=3"   title="全部" class="nor">全部</a>
   ...
   <table cellspacing="0" summary="" class="tab" id="myTable">
    ...
    <c:forEach items="${tbList}" var="tb">
     <tr>
      ...
      <td>
       <c:if test="${tb.order.isPaid==0}">
        未付款
       </c:if>
       <c:if test="${tb.order.isPaid==1}">
        已付款
       </c:if>
      </td>
      <td class="nwp">
       <fmt:formatDate value="${tb.order.orderTime}" pattern="yyyy-MM-dd HH:mm:ss"></fmt:formatDate>
      </td>
      <td class="nwp">
       <fmt:formatDate value="${tb.order.updateTime}" pattern="yyyy-MM-dd HH:mm:ss"></fmt:formatDate>
      </td>
      <td class="nwp">
       未付款单
      </td>
      <td><a href="${path }/order/viewDetail.do?taskId=${tb.task.id}&orderId=${tb.order.orderId}&dirName=orderPay">查看</a></td>
     </tr>
    </c:forEach>
   </table>
